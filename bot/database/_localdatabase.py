...
#     ) -> User:
#     user = session.query(User).filter(User.id == telegram_user_id).one_or_none() or \
#         User(telegram_user_id=telegram_user_id)
#     if sign_language_code:
#         try:
#             user.sign_language_id = get_language(code=sign_language_code).id
#         except AttributeError as e:
#             raise TypeError(f'sign language code: {sign_language_code!r} does not exists') from e
#     if first_name:
...
#         book = get_book(language_code=edition.language.code, booknum=booknum, edition_id=edition.id)
#         if not book:
#             books.append(
#                 Book(
...
#         q = (q
#              .join(Bible, Bible.id == Book.edition_id)
...
#         q = q.filter(Bible.id == edition_id)
#     return q.one_or_none()
